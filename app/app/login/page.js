            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {mode === 'register' && (
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 500,
                      ...
                      letterSpacing: '0.04em' }}>USERNAME</label>
                    <div style={{ position: 'relative' }}>
                      <div style={{ position: 'absolute', left: 14, top: '50%',
                        transform: 'translateY(-50%)', fontSize: 16, opacity: 0.5 }}>👤</div>
                      <input type="text" value={username}
                        onChange={e => setUser(e.target.value)}
                        placeholder="your_username" required
                        style={{ width: '100%', padding: '0.875rem 1rem 0.875rem 2.75rem',
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 500,
                    color: '#8888aa', display: 'block', marginBottom: 6,
                    letterSpacing: '0.04em' }}>EMAIL ADDRESS</label>
                  <div style={{ position: 'relative' }}>
                    <div style={{ position: 'absolute', left: 14, top: '50%',
                      transform: 'translateY(-50%)', fontSize: 16, opacity: 0.5 }}>✉️</div>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between',
                    ...
                    <label style={{ fontSize: 12, fontWeight: 500,
                      color: '#8888aa', letterSpacing: '0.04em' }}>PASSWORD</label>
                    {mode === 'login' && (
                      <button type="button"
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input type={showPass ? 'text' : 'password'}
                      ...
                      style={{ width: '100%', padding: '0.875rem 3rem 0.875rem 2.75rem',
                        background: '#080818', border: '1px solid #1a1a35',
                        borderRadius: 12, color: 'white', fontSize: 14,
                        outline: 'none', fontFamily: 'inherit' }}
                      onFocus={e => e.target.style.borderColor = '#4a9eff'}
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? 'Please wait...' : mode === 'login'
                    ? '→  Sign in to MindTrack' : '→  Create your account'}
                </button>
              </div>
            </form>
